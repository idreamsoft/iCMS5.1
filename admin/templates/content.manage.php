...
<div class="position">当前位置：管理中心&nbsp;&raquo;&nbsp;<?php echo $model['name'];?>管理<?php echo $position;?></div>
  ...
  <table class="adminlist">
    <tr>
      <td class="tipsblock"><select name="at" id="at">
          <option value="-1"> == <?php echo $model['name'];?>属性 == </option>
          <option value="0">普通<?php echo $model['name'];?>[type='0']</option>
          <?php echo contentype($model['table']) ; ?>
        </select>
        <select name="fid" id="fid">
          <option value="0"> == 按栏目 == </option>
          <?php echo $forum->select($_GET['fid'],0,1,NULL,$mid) ; ?>
        </select>
...
<form action="<?php echo __ADMINCP__; ?>=content" method="post" target="iCMS_FRAME">
  <table class="adminlist">
    <thead>
      <tr>
    ...
    <tbody id="alist">
      <?php for($i=0;$i<$_count;$i++){
		...
		$iurl=$this->iCMS->iurl('content',array($rs[$i],$F,$model));
      ...
      <tr class="row<?php echo ($i%2) ; ?>" id="aid<?php echo $rs[$i]['id'] ; ?>">
        ...
        <td><input type="text" name="orderNum[<?php echo $rs[$i]['id'] ; ?>]" value="<?php echo _int($rs[$i]['orderNum']); ?>" style="width:24px;border:1px #F6F6F6 solid;"/></td>
        <td title="<?php echo $rs[$i]['title'] ; ?>&#10;<?php echo $model['name'];?>ID:<?php echo $rs[$i]['id'] ; ?>&#10;点击数:<?php echo $rs[$i]['hits'] ; ?>&#10;评论数:<?php echo $rs[$i]['comments'] ; ?>&#10;顶:<?php echo $rs[$i]['good'] ; ?>&#10;踩:<?php echo $rs[$i]['bad'] ; ?>"><div style="height:20px;width:360px;overflow:hidden;">
          ...
          <div class="row-actions">
            <div class="do" style="display:none;">
          	<?php if($rs[$i]['status']=="0"||$rs[$i]['status']=="1"){  ?>
          	  <a href="<?php echo __ADMINCP__; ?>=content&do=status&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&id=<?php echo $rs[$i]['id'] ; ?>&s=<?php echo $rs[$i]['status'] ; ?>" target="iCMS_FRAME"><?php echo $type?$_ptxt[$rs[$i]['status']]:'转为草稿' ; ?></a> | 
           <?php } ?>
              <?php if($fid && $fid!=$rs[$i]['fid'] && $_GET['sub']!='on'){?>
              	<a href="<?php echo __ADMINCP__; ?>=content&do=delvlink&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&id=<?php echo $rs[$i]['id'] ; ?>&fid=<?php echo $fid ; ?>" onclick="return confirm('删除<?php echo $rs[$i]['title'] ; ?>此栏目的虚链接?');" target="iCMS_FRAME" class="del">删除</a>
              <?php }else{?>
	              <?php if ($F['mode'] && strstr($F['contentRule'],'{PHP}')===false && $rs[$i]['status']=="1" && empty($ourl)){  ?>
		              <a href="<?php echo __ADMINCP__; ?>=content&do=updateHTML&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&id=<?php echo $rs[$i]['id'] ; ?>" target="iCMS_FRAME" class="update"><?php echo file_exists($htmlurl)?"更新静态":"生成静态" ; ?></a> |
	              <?php }  ?>
              <!--<a href="<?php echo __ADMINCP__; ?>=content&do=copy&id=<?php echo $rs[$i]['id'] ; ?>"  class="copy">复制</a> | -->
              	<a href="<?php echo __ADMINCP__; ?>=content&do=add&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&id=<?php echo $rs[$i]['id'] ; ?>" class="edit" title="编辑此<?php echo $model['name'];?>">编辑</a> | <!--<a href="<?php echo __ADMINCP__; ?>=push&do=add&id=<?php echo $rs[$i]['id'] ; ?>&TB_iframe=true&height=480&width=800" class="thickbox" title="推送">推送</a> | -->
	              <?php if($rs[$i]['status']=="1"){  ?>
	              	  <a href="<?php echo $rs[$i]['url'] ; ?>"  target="_blank" class="view" title="查看“<?php echo $rs[$i]['title'] ; ?>”">查看</a> | 
	              	  <a href="<?php echo $this->iCMS->config['publicURL']; ?>/comment.php?indexId=<?php echo $rs[$i]['id'] ; ?>&mId=<?php echo $mid; ?>&sortId=<?php echo $rs[$i]['fid'] ; ?>" target="_blank" title="查看评论" class="comment">评论</a> | 
	              <?php }  ?>
	              <?php if($rs[$i]['status']=="2"){  ?>
	              	  <a href="<?php echo __ADMINCP__; ?>=content&do=recover&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&id=<?php echo $rs[$i]['id'] ; ?>" target="iCMS_FRAME" class="del" title='还原此<?php echo $model['name'];?>'>还原</a> | 
	              	  <a href="<?php echo __ADMINCP__; ?>=content&do=del&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&id=<?php echo $rs[$i]['id'] ; ?>" target="iCMS_FRAME" class="del" onclick="return confirm('确定要删除“<?php echo HTML2JS($rs[$i]['title']) ; ?>”?');" title='永久删除此<?php echo $model['name'];?>,此操作不可恢复'>永久删除</a>
	              <?php }else{?>
	              	 <a href="<?php echo __ADMINCP__; ?>=content&do=trash&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&id=<?php echo $rs[$i]['id'] ; ?>" target="iCMS_FRAME" class="del" title='移动此<?php echo $model['name'];?>到回收站'>删除</a>
	              <?php } ?>
              <?php }  ?>
            </div>
          </div></td>
        <td><a href="<?php echo __ADMINCP__; ?>=content&do=manage&mid=<?php echo $mid ; ?>&table=<?php echo $model['table'] ; ?>&fid=<?php echo $rs[$i]['fid'] ; ?><?php echo $uri ; ?>"><?php echo $F['name'] ; ?></a></td>
        ...
        <td><?php echo get_date($rs[$i]['pubdate'],'Y-m-d H:i');?><br />
      <?php if ($F['mode'] && strstr($F['contentRule'],'{PHP}')===false && $rs[$i]['status']=="1" && empty($ourl)){
    ...
    <tr>
      <td colspan="7"><input type="checkbox" name="chkall" id="chkall" class="checkbox" onclick="iCMS.checkAll('prefix', this.form, 'id')" />
        ...
        <select name="do" id="do" onChange="doaction(this);">
          ...
          <option value="contentype">设置<?php echo $model['name'];?>属性</option>
  ...
  <div class="tipsdiv" id="contentype-div">
	  <table class="adminlist" style="width:210px;">
	    <thead>
	      <tr>
	        <th><span style="float:right;margin-top:4px;" class="close" parent="contentype-div"><img src="admin/images/close.gif" /></span>选择<?php echo $model['name'];?>属性</th>
	      </tr>
	    </thead>
	    <tr>
