...
<div class="pwrap">
  <div class="phead">
    <div class="phead-inner">
      <div class="phead-inner">
        <h3 class="ptitle"><span>我的文章</span></h3>
          ...
          <form method="get" action="<?php echo __SELF__ ; ?>" name="form3">
            ...
            <div class="inner">
              <select id="fid" name="fid">
                ...
                <?php echo $forum->user_select(0,0,1,'all') ; ?>
              </select>
      ...
      <table class="datatable">
        ...
        <tbody>
          <?php if($rs)for($i=0;$i<$_count;$i++){
				...
				$rs[$i]['url']=$rs[$i]['status']?$iurl->href:"javascript:iCMS.showDialog('".__USERCP__.'=article&do=preview&id='.$rs[$i]['id']."','preview','预览');\" target=\"iCMS_FRAME";
				if(stristr($rs[$i]['url'], 'show.php')) {
				    $rs[$i]['url']='../'.$rs[$i]['url'];
				}
	  ?>
          <tr id="tr-<?php echo $rs[$i]['id'] ; ?>">
            ...
            <td><?php if($rs[$i]['status']=="1"){ ?>
              ...
              <?php } ?></td>
            <td><a href="<?php echo __USERCP__; ?>=article&do=add&id=<?php echo $rs[$i]['id'] ; ?>" class="edit" title="编辑此文章">编辑</a> | <a href="<?php echo __USERCP__; ?>=article&do=del&id=<?php echo $rs[$i]['id'] ; ?>" target="iCMS_FRAME" class="del" onclick="return confirm('确定要删除“<?php echo HTML2JS($rs[$i]['title']) ; ?>”?');" title='删除此文章,此操作不可恢复'>删除</a> | <a target="_blank" href="<?php echo $rs[$i]['url'] ; ?>">
          ...
          <?php }else{ ?>
          <tr>
            <td colspan="6"><div class="nodata">当前没有任何文章.</div></td>
          </tr>
          <?php }  ?>
        </tbody>
      </table>
